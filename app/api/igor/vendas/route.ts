...
export async function GET(req: Request) {
  ...
  const vendas = sales.map((s) => ({
    ...
    status: s.status, payment: s.payment,
    totalCents: s.totalCents, descontoVendaCents: s.descontoVendaCents,
    buyerName: s.nomeComprador, nucleo: s.nucleo,
    sellerUserId: s.sellerUserId, sellerName: usersMap[s.sellerUserId] ?? "Desconhecido",
    itens: s.items.map((it) => {
      ...
      return { id: it.id, nome, productId: it.productId, variantId: it.variantId, comboId: it.comboId, qty: it.qty, unitCents: it.unitCents, totalCents: it.totalCents }
    }),
  }))
...
export async function PATCH(req: Request) {
  ...
  if ("sellerUserId" in body && body.sellerUserId) data.sellerUserId = String(body.sellerUserId)

  const itensPatch = Array.isArray(body.itens) ? body.itens as Array<{ id: number; qty?: number; unitCents?: number }> : []
  ...
  await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    for (const item of itensPatch) {
      ...
      if (item.qty != null && item.unitCents != null) itemData.totalCents = Number(item.qty) * Number(item.unitCents)
      if (Object.keys(itemData).length > 0) await tx.saleItem.update({ where: { id: item.id }, data: itemData })
    }
  })

  const updated = await prisma.sale.findUnique({ where: { id }, include: { items: { include: { product: true, variant: true, combo: true } } } })
...
export async function DELETE(req: Request) {
  ...
  const venda = await prisma.sale.findUnique({ where: { id }, include: { items: true } })
  if (!venda) return NextResponse.json({ error: "Venda não encontrada" }, { status: 404 })
  if (venda.status === "CANCELED") return NextResponse.json({ error: "Venda já cancelada" }, { status: 409 })

  await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    await tx.sale.update({ where: { id }, data: { status: "CANCELED" } })
    for (const it of venda.items) {
      if (it.variantId) {
        await tx.productVariant.update({ where: { id: it.variantId }, data: { stockOnHand: { increment: it.qty } } })
        await tx.variantStockMovement.create({ data: { variantId: it.variantId, type: "IN", qty: it.qty, actorUserId: userId!, note: `[IGOR] Cancelamento venda ${venda.code}` } })
      } else if (it.productId) {
        await tx.product.update({ where: { id: it.productId }, data: { stockOnHand: { increment: it.qty } } })
        await tx.stockMovement.create({ data: { productId: it.productId, type: "IN", qty: it.qty, actorUserId: userId!, note: `[IGOR] Cancelamento venda ${venda.code}` } })
      }
    }
  })
