...
function buildPixPayload(valor: number): string {
  ...
  const payload = [
    ...
    "52040000",
    "5303986",
    `54${padN(formattedAmount.length)}${formattedAmount}`,
    "5802BR",
    `59${padN(cleanName.length)}${cleanName}`,
    `60${padN(cleanCity.length)}${cleanCity}`,
    txidField,
    "6304",
  ].join("")
...
// ─── Modal QR Code PIX ────────────────────────────────────────────────────────
function PixQrModal({
  open, onOpenChange, total, onConfirmar, carregando,
}: {
  open: boolean
      ...
      <DialogContent className="max-w-sm rounded-3xl p-0 overflow-hidden border-0 shadow-2xl">
        ...
        <div className="flex flex-col items-center gap-4 p-5">
          ...
          <div className="flex w-full gap-2">
            ...
            <Button
              className="flex-1 rounded-full bg-gradient-to-r from-red-600 to-rose-500 font-bold hover:from-red-700 hover:to-rose-600 shadow-md"
              onClick={onConfirmar}
              disabled={carregando}
            >
      ...
      <DialogContent className="max-w-sm rounded-3xl p-0 overflow-hidden border-0 shadow-2xl">
        ...
        <div className="flex flex-col gap-4 p-5">
          ...
          <div className={`rounded-2xl border-2 p-4 text-center transition-all ${
            ...
            {!valorStr ? (
              ...
              <p className="text-base font-bold text-red-500">
                Faltam R$ {Math.abs(troco).toFixed(2).replace(".", ",")}
              </p>
            )}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button
              className="flex-1 rounded-full bg-gradient-to-r from-emerald-600 to-teal-500 font-bold hover:from-emerald-700 hover:to-teal-600 shadow-md disabled:opacity-40"
              onClick={onConfirmar}
              disabled={!trocoValido || carregando}
            >
              {carregando ? "Processando..." : (
              ...
              <div className="relative">
                ...
                <Select value={nucleo} onValueChange={setNucleo}>
                  ...
                  <SelectContent className="rounded-2xl">
                    <SelectItem value="nao_informado" className="rounded-xl text-gray-400 italic">
                      Não informado
                    </SelectItem>
                    {NUCLEOS.map((n) => (
                      <SelectItem key={n} value={n} className="rounded-xl">
                        {n}
                      </SelectItem>
                    ))}
                  </SelectContent>
